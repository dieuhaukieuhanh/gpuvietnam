...
for (const line of readFileSync(".env.local", "utf8").split(/\r?\n/)) {
  ...
  const k = t.slice(0, i).trim(); if (!process.env[k]) process.env[k] = v;
}
process.env.GPU_CLORE_ONLY = "true";
const { CloreClient } = await import(pathToFileURL(join(process.cwd(), "src/lib/gpu/providers/clore/clore-client.js")).href);
...
try {
  const raw = await client.createInstance({
    gpuLine: "rtx3090",
    plan: "starter",
    label: "gv-amp-fix-" + Date.now().toString(36).slice(-5),
    env: {
...
writeFileSync("tmp/clore-amp-fix.json", JSON.stringify(report, null, 2));
