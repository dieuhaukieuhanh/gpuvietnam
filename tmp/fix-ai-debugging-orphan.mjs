...
const p = new URL('../../AI_DEBUGGING.md', import.meta.url);
let s = fs.readFileSync(p, 'utf8');
if (s.charCodeAt(0) === 0xfeff) s = s.slice(1);

const marker = '## Recovering Clore Orphan Orders';
const idx = s.indexOf(marker);
let head = idx >= 0 ? s.slice(0, idx) : s;
head = head.replace(/\n+$/, '\n\n');
...
const section = `## Recovering Clore Orphan Orders
...
fs.writeFileSync(p, head + section, 'utf8');
console.log('AI_DEBUGGING.md updated', fs.statSync(p).size);
